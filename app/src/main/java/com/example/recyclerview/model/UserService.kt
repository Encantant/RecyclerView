...
class UsersService {
    ...
    init {
        ...
        users = (1..50).map { User(
            ...
            company = faker.company().name(),//it.toString(),//
            photo = IMAGES[it % IMAGES.size],
            isLiked = false
        ) }.toMutableList()
    }

    fun addUser(id: Int, user: User){//id: Long, name: String, company: String, photo: String, like:Boolean) {
        //val a = id.toInt()-1
        users.add(id,user)
        notifyChanges()
    }
    ...
    fun getUserCompany(id: Int): String {
    ...
    fun getUserLiked(id: Int): Boolean {
        return users[id].isLiked
    }

    fun deleteUserStrict(id: Int) {
        users.removeAt(id)
        notifyChanges()
    }
    ...
    fun likeUser(user: User) {
        val index = users.indexOfFirst { it.id == user.id } // Находим индекс человека в списке
        if (index == -1) return // Останавливаемся, если не находим такого человека
        //users = ArrayList(users) // Создаем новый список
        users[index] = users[index].copy(isLiked = !users[index].isLiked) // Меняем значение "лайка" на противоположное
        notifyChanges()
    }
